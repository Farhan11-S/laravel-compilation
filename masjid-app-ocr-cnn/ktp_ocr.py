...
def imageCropper(image, img_gray, filename, dir, goodResultLatest, goodResultOld):
    ...
    image = cv2.threshold(blur, 0, 255, cv2.THRESH_TRUNC + cv2.THRESH_OTSU)[1]
    ...
    for (i, c) in enumerate(cnts):
        ...
        if (w > 360 and h > 240) or (w > 300 and h < 30):
            ...
            if w > 425 and h > 200:
                biodata = pytesseract.image_to_string(crop_img, lang="Arialv2.4", config='--psm 4 --oem 3')
            
            if w > 300 and h < 30:
                nik = pytesseract.image_to_string(crop_img, lang="OCRA-11-02-2024v1.4", config='--psm 7 --oem 3')
