...
use App\Services\AutoPost\Telegram\Api as TelegramApi;
use App\Services\AutoPost\Whatsapp\Api as WhatsappApi;
    ...
    public static function Telegram($text, $attachment = '', $inline_keyboard = '')
    {
        ...
        if ($attachment) {
            switch ($attachment['type']) {
                case 'photo':
                    $result = TelegramApi::sendPhoto($attachment['file'], $text, $inline_keyboard);
                    break;
                case 'audio':
                    ...
                    $result = TelegramApi::sendAudio($attachment['file'], $text, $duration, $performer, $title, $inline_keyboard);
                    break;
                case 'document':
                    $result = TelegramApi::sendDocument($attachment['file'], $text, $inline_keyboard);
                    break;
                case 'video':
                    $duration = isset($attachment['duration']) ? $attachment['duration'] : '';
                    $width = isset($attachment['width']) ? $attachment['width'] : '';
                    $height = isset($attachment['height']) ? $attachment['height'] : '';
                    $result = TelegramApi::sendVideo($attachment['file'], $duration, $width, $height, $text, $inline_keyboard);
                    break;
                case 'voice':
                    $duration = isset($attachment['duration']) ? $attachment['duration'] : '';
                    $result = TelegramApi::sendVoice($attachment['file'], $text, $duration, $inline_keyboard);
                    break;
                case 'media_group':
                    $result = TelegramApi::sendMediaGroup(json_encode($attachment['files']));
                    break;
                case 'location':
                    $live_period = isset($attachment['live_period']) ? $attachment['live_period'] : '';
                    $result = TelegramApi::sendLocation($attachment['latitude'], $attachment['longitude'], $live_period, $inline_keyboard);
                    break;
                case 'venue':
                    $foursquare_id = isset($attachment['foursquare_id']) ? $attachment['foursquare_id'] : '';
                    $result = TelegramApi::sendVenue($attachment['latitude'], $attachment['longitude'], $attachment['title'], $attachment['address'], $foursquare_id, $inline_keyboard);
                    break;
                case 'contact':
                    $last_name = isset($attachment['last_name']) ? $attachment['last_name'] : '';
                    $result = TelegramApi::sendContact($attachment['phone_number'], $attachment['first_name'], $last_name, $inline_keyboard);
                    break;
            }
        } else {
            $result = TelegramApi::sendMessage($text, $inline_keyboard);
        }

        return $result;
    }

    public static function Whatsapp(string $message, $priority = null)
    {
        return WhatsappApi::messageChannel(Config::get('wassenger.authorisation.channel_id'), $message, $priority)->send();
    }
